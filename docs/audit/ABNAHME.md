...
| 10 | Erfolgs-Hinweis bei Ankündigung wird überlagert | NIEDRIG | **BEHOBEN, UNGETESTET** | `26fa57c`: `onSaved` hängt jetzt am OK-Knopf des Alerts (`[{ text: 'OK', onPress: onSaved }]`), der Erfolgsweg kehrt vorher zurück — der Editor klappt erst zu, wenn der Hinweis quittiert ist. Muster von `items/new.tsx:105-111` übernommen |
...
| 10 | `export type PPTheme` ohne Verwender | **BEHOBEN, UNGETESTET** | `49a7d51`: Der Typ ist ganz entfernt, nicht nur der `export` — er hatte repoweit keinen Verwender. Ein Kommentar an der Stelle nennt `typeof PP` als Ersatz |
...
| 2 | `campaignFactorsLabel` ohne Aufruf | **BEHOBEN, UNGETESTET** | `49a7d51`: Die Funktion hat jetzt einen Aufrufer — `actions.tsx` nutzt sie für die Faktorenzeile der Listendarstellung, statt den Fließtext ein drittes Mal von Hand zu bauen (siehe D1) |
| 3 | Zwei Typen unnötig exportiert (`Motivation`, `MotivationInput`) | **BEHOBEN, UNGETESTET** | `49a7d51`: beide nur noch `interface` statt `export interface`. Nicht gelöscht — `motivationFor()` braucht sie in der Signatur; die Aufrufstelle in `index.tsx` leitet ihre Typen daraus ab |
...
| 8 | Zwei Typ-Exporte in `types.ts` (`Role`, `BadgeKind`) | **BEHOBEN, UNGETESTET** | `49a7d51`: `pb.ts` importiert jetzt `Role` aus `types.ts`, statt die drei Rollen ein zweites Mal wörtlich aufzuzählen — wie vom Bericht empfohlen, also umgestellt statt gelöscht. `BadgeKind` hat mit `types.ts:79` (`kind?: BadgeKind`) einen Verwender im eigenen Modul und bleibt |
| D1 | Aktions-Faktoren in drei Fassungen | **BEHOBEN, UNGETESTET** | `49a7d51`: `campaignFactorRows` gelöscht, die Liste in `actions.tsx` nutzt `campaignFactorsLabel` aus `format.ts`. Vorher geprüft, dass die Fassungen dasselbe tun — Unterschied waren ein Wort (lokal „Kommen" statt „Vorbeikommen", jetzt einheitlich) und `.replace('.', ',')` gegen `toLocaleString('de-DE')`, für die Faktoren 1–3 wertgleich. `factorLabel` bleibt: Es beschriftet die Auswahlknöpfe, nicht die Liste |
...
| M-2 | TestFlight lädt hoch, bevor die Hinweise feststehen | MITTEL | **BEHOBEN, UNGETESTET** | `cfb25d2`: „Letzten gebauten Stand ermitteln" und „Testhinweise zusammenstellen" stehen jetzt vor dem Upload. Der Befund trägt: Scheitert `release-notes.py`, ist der Build bei Apple angekommen und die Nummer verbraucht — er stünde unbeschriftet in TestFlight. Dass der letzte Lauf erfolgreich war, widerlegt ihn nicht; er beschreibt den Fehlerfall. Das Vorziehen ändert das Ergebnis der Abfrage nicht, weil sie den letzten *erfolgreichen* Lauf sucht und der gerade laufende nicht dazuzählt |
| M-3 | `docker-compose.yml` ohne Ressourcen- und Protokollgrenzen | MITTEL | **BEHOBEN, UNGETESTET** | `e5ca543`: `logging` (`max-size: 10m`, `max-file: 3`), `mem_limit: 512m`, `cpus: 1.0` — in **beiden** Fassungen, also auch in `docker-compose.portainer.yml`, das in Produktion läuft. Die Protokollgrenze ist der praktisch wichtigere Teil: unbegrenzte JSON-Logs füllen die Platte und reißen die übrigen Stacks mit. Grundlage gemessen statt geschätzt: Der Container belegte 19,6 MB und sah bis dahin die vollen 24 GB des Hosts |
| M-4 | Versionsangaben und CHANGELOG-Rückstand | MITTEL | **BEHOBEN** | `38b3725`: Der Block ist als `## [1.0.0 (33)] – 2026-09-14` geschlossen, darüber ein leeres `## [Unreleased]`. Die Zusammenführung auf vier Rubriken (Neu / Geändert / Behoben / Sonstiges) war bereits vorher erledigt — der Teil des Befunds ist damit hinfällig, offen war nur noch der Abschluss zu einem Versionsabschnitt |
| N-1 | `upload-play.py` fängt `urllib.error.HTTPError` ohne Import | NIEDRIG | **BEHOBEN, UNGETESTET** | `1df7323`: `import urllib.error` ergänzt, mit Begründung an der Stelle |
| N-2 | `asc-build-number.py` verwirft nicht-numerische Nummern | NIEDRIG | **BEHOBEN, UNGETESTET** | `1df7323`: Das Skript bricht jetzt mit Exit 1 und Meldung ab, statt die Nummer still zu verwerfen und womöglich eine schon vergebene auszurechnen. Dazu eine Warnung, wenn die Abfrage am `limit=200` anschlägt. Gegen einen lokalen Server geprüft: `1.0.0.23` → Abbruch ohne Ausgabe, `33`/`7` → `build=34` |
| N-3 | Zwei Skripte ohne Fehlerbehandlung an der Netzgrenze | NIEDRIG | **BEHOBEN, UNGETESTET** | `1df7323`: `asc-build-number.py` (`hole()`) und `play-version.py` (`oeffne()`) geben bei HTTP- und Verbindungsfehlern den Antwortkörper aus und enden mit Exit 1 statt mit einem Stapelabzug. Das Aufräumen im `finally` von `play-version.py` läuft bewusst ohne diese Behandlung (`streng=False`), damit es den eigentlichen Fehler nicht verdeckt. Gegen einen lokalen Server geprüft: 403 mit Grund im Körper, toter Host, Erfolgsfall |
